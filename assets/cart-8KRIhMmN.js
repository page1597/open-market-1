import"./modulepreload-polyfill-B5Qt9EMX.js";const u="https://openmarket.weniv.co.kr";let l=[];const b=()=>{console.log("header.html"),fetch("header.html").then(t=>t.text()).then(t=>{document.querySelector("header").outerHTML=t;const e=document.createElement("script");e.src="/open-market-service/src/js/header.js",document.body.appendChild(e)})},f=()=>{fetch("footer.html").then(t=>t.text()).then(t=>{document.querySelector("footer").outerHTML=t})};document.addEventListener("DOMContentLoaded",()=>{b(),f(),S()});const g=t=>{const e=document.querySelector("#select-all");e.addEventListener("click",()=>{const n=e.checked;document.querySelectorAll('input[type="checkbox"]:not(#select-all)').forEach(c=>{c.checked=n,t&&h(t,n)}),v()})},S=async()=>{try{const t=await C();if(!t)return;const e=document.querySelector(".cart-items-container");for(const n of t.results){const o=await L(n.product_id);if(o){g(o);const c=_(o,n.quantity,n.cart_item_id);e.appendChild(c)}}}catch(t){console.error("장바구니 그리기 오류:",t)}},_=(t,e,n)=>{const o=document.createElement("article");o.className="cart-item";const c=new Intl.NumberFormat("ko-KR");return o.innerHTML=`
    <div>
  ...
  `,k(o,t,e,n),o},k=(t,e,n,o)=>{const c=t.querySelector(`#remove-${e.product_id}`),r=t.querySelector(`#minus-${e.product_id}`),i=t.querySelector(`#plus-${e.product_id}`),s=t.querySelector(`#select-${e.product_id}`);c.addEventListener("click",()=>{q(o)}),r.addEventListener("click",()=>{y(e,o)}),i.addEventListener("click",()=>{y(e,o)}),s.addEventListener("input",()=>h(e,s.checked))},h=(t,e)=>{console.log(t);const n=parseInt(document.querySelector(`#quantity-${t.product_id}`).value,10);l=l.filter(o=>o.id!==t.product_id),e&&l.push({id:t.product_id,price:t.price*n,discountPrice:0,shippingFee:t.shipping_fee}),console.log(l),v()},v=()=>{const t=document.querySelector(".total-count"),e=t.querySelector(".product-price .value em"),n=t.querySelector(".shipping-fee .value em"),{finalOrderPrice:o,finalShippingFee:c}=l.reduce((r,i)=>(r.finalOrderPrice+=i.price,r.finalShippingFee+=i.shippingFee,r),{finalOrderPrice:0,finalShippingFee:0});e.textContent=o.toLocaleString(),n.textContent=c.toLocaleString(),t.querySelector(".total-price .value em").textContent=(o+c).toLocaleString()},$=(t,e)=>{const n=document.createElement("article");return n.className=t,n.innerHTML=e,document.body.appendChild(n),n},y=(t,e)=>{let n=parseInt(document.querySelector(`#quantity-${t.product_id}`).value,10);const o=`
